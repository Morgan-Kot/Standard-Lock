        ...
        .pick_files()
    {
        ...
        for path in paths {
            ...
            if state.config.lock_all {
                if !state.config.exceptions.contains(&p_str) {
                    state.config.exceptions.push(p_str);
                }
            } else {
                if !state.config.explicit_locked_paths.contains(&p_str) {
                    state.config.explicit_locked_paths.push(p_str);
                }
            }
        }
        return count;
