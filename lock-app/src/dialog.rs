...
//! Deliberately raw Win32 rather than a GUI framework: no GPU context,
//! no extra runtime, so it opens in milliseconds and idles at only a
//! few MB of RAM while it's on screen - which is most of the whole
//! app's memory footprint, since it's a short-lived process.
...
/// Shows the password prompt for `file_name`. Returns true only if the
/// correct password was entered. Blocks the calling thread until the
/// dialog is closed (this process has nothing else to do meanwhile).
pub fn prompt_password(file_name: &str, hash: &str) -> bool {
    unsafe {
        ...
        let title_wide = to_wide(&format!("Standard Lock"));
        ...
        while GetMessageW(&mut msg, None, 0, 0).as_bool() {
            // Enter/Escape work anywhere in the dialog, not just when
            // the buttons have focus.
...
unsafe fn make_static(
    ...
    .unwrap_or_default();
    let _ = bold; // font customization intentionally left minimal/system-default
    hwnd
}
